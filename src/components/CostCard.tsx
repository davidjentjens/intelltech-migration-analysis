...
const CostCard: React.FC<CostCardProps> = ({ platform, data }) => (
  <div className={`bg-white rounded-lg border-2 p-4 h-full ${
    platform.includes('GitLab') ? 'border-orange-500' : 
    platform.includes('GitHub') ? 'border-gray-900' : 'border-gray-400'
  }`}>
    <div className="flex items-center gap-2 mb-3">
      ...
      <div className="ml-auto flex gap-1">
        ...
        {(platform.includes('Copilot') || platform.includes('Ultimate') || platform.includes('Enterprise Cloud')) && 
          <Zap className="w-4 h-4 text-yellow-500" title="IA Integrada" />
        }
      </div>
    </div>
    <div className="space-y-2 text-sm">
      <div><span className="font-medium">Mensal:</span> {data.mensal}</div>
      <div><span className="font-medium">Anual:</span> {data.anual}</div>
